...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, int algo, bool mm)
{
  if (RegTest()) return Params().ProofOfWorkLimit().GetCompact();
    ...
    if (workAlgo == 0) {
      ...
      // Post 8mPoW fork
      return DarkGravityWave(pindexLast,algo,mm);
    }
}

unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast)
{
  return GetNextWorkRequired(pindexLast, ALGO_SCRYPT, false);
}
...
bool AcceptBlock(CBlock& block, CValidationState& state, CDiskBlockPos* dbp)
{
	...
	int block_algo = GetAlgo(block.nVersion);
	bool mm = false;
	if (block.IsAuxpow()) mm = true;
	unsigned int next_work_required = GetNextWorkRequired(pindexPrev, block_algo, mm);
